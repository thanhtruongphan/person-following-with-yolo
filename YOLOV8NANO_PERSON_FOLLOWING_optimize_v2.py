...
class PersonDetector:
    def __init__(self, input_width=320, input_height=320):
        ...
        # Pre-allocate GPU memory for color conversion
        self.gpu_stream = cv2.cuda.Stream()
        self.gpu_rgb = cv2.cuda_GpuMat()
        self.gpu_hsv = cv2.cuda_GpuMat()
        
        # Set the specific HSV range
        self.lower_hsv = np.array([30, 50, 100], dtype=np.uint8)
        self.upper_hsv = np.array([90, 255, 255], dtype=np.uint8)
        
        # Initialize optimized YOLOv8 model
        ...
        self.resized_frame = np.zeros((input_height, input_width, 3), dtype=np.uint8)
        self.detection_mask = np.zeros((input_height, input_width), dtype=np.uint8)

    def _initialize_model(self):
        """Initialize and optimize YOLO model"""
        try:
            # Load model
            ...
            print(f"Failed to initialize TensorRT model: {e}")
            print("Falling back to standard model")
            model = YOLO("yolov8n.pt")
            model.fuse()
            return model

    def _calculate_color_ratio(self, roi):
        """Optimized color ratio calculation without numba"""
    ...
    def check_color_in_roi(self, hsv_frame, x, y, w, h):
        ...
        y1 = max(0, y)
        y2 = min(hsv_frame.shape[0], y + h)
        x1 = max(0, x)
        x2 = min(hsv_frame.shape[1], x + w)
        
        if y2 <= y1 or x2 <= x1:
            return False
            
        roi = hsv_frame[y1:y2, x1:x2]
        return self._calculate_color_ratio(roi) > 0.1
    ...
    def detect(self, frame, hsv_frame):
        ...
        # Skip frames if we have recent detection
        ...
        try:
            ...
            gpu_resized = cv2.cuda.resize(gpu_frame, (self.input_width, self.input_height))
            gpu_resized.download(self.resized_frame)
        except Exception as e:
            print(f"GPU resize failed, falling back to CPU: {e}")
            self.resized_frame = cv2.resize(frame, (self.input_width, self.input_height))
        ...
        results = self.model(
            ...
            classes=0,
            verbose=False,
            half=True  # Enable FP16 inference
        )
        ...
        # Process detections
        if len(results) > 0:
            # Get all detections at once for batched processing
            all_boxes = results[0].boxes
            
            if len(all_boxes) > 0:
                # Convert boxes to numpy for faster processing
                xyxys = all_boxes.xyxy.cpu().numpy()
                confs = all_boxes.conf.cpu().numpy()
                
                for idx, (box, conf) in enumerate(zip(xyxys, confs)):
    ...
    def __del__(self):
        ...
        self.gpu_rgb.release()
        self.gpu_hsv.release()
        torch.cuda.empty_cache()
